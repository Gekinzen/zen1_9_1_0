...
show_decorations_menu() {
    ...
    if [ "$BAR_TITLE_ENABLED" = "true" ]; then
        TITLE_STATUS="󰄬 Show Titles"
        TITLE_ICON="󰅙"
    else
        TITLE_STATUS="󰅙 Hide Titles"
        TITLE_ICON="󰄬"
    fi
    
    CHOICE=$(echo -e "${BAR_STATUS}\n${BAR_ICON} Toggle Bars\n${TITLE_STATUS}\n${TITLE_ICON} Toggle Titles\n󰘖 Bar Height (${BAR_HEIGHT}px)\n󰖰 Minimized Windows ($MIN_COUNT)\n󰚰 Update Colors\n󰁍 Back" | \
        rofi -dmenu -i \
    ...
    case "$CHOICE" in
        *"Toggle Bars"*)
            ...
            ;;
        *"Toggle Titles"*)
            ~/.config/hypr/scripts/hyprbars-manager.sh title
            show_decorations_menu
            ;;
        ...
        *"Minimized"*) 
            ...
            ;;
        *"Update Colors"*)
            ~/.config/hypr/scripts/hyprbars-manager.sh colors
            show_decorations_menu
            ;;
