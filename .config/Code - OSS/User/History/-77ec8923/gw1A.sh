...
update_colors() {
    ...
    # Update hyprbars colors in hyprland.conf
    sed -i "s/bar_color = rgb(.*)/bar_color = rgb(${background#\#})/" "$HYPR_CONFIG"
    sed -i "s/col.text = rgb(.*)/col.text = rgb(${foreground#\#})/" "$HYPR_CONFIG"
    
    notify-send "󰚰 Hyprbars" "Colors updated from pywal" -i preferences-desktop-theme
    hyprctl reload
}
