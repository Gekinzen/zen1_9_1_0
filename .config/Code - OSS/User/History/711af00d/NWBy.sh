...
main() {
    # Store the currently focused window BEFORE opening wofi
    ...
    # CRITICAL: Return focus to the previous window
...
EOF
    ...
    # FINAL: Ensure focus is back to window so waybar becomes clickable again
    sleep 0.3
    if [ -n "$current_window" ] && [ "$current_window" != "null" ]; then
        hyprctl dispatch focuswindow address:$current_window
    fi
