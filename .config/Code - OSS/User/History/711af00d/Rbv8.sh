...
if [ -x ~/.config/hypr/scripts/dock-manager.sh ]; then
    ...
    # Map layout to dock mode
    if [[ "$layout" == "float" ]]; then
        dock_mode="float"
    else
        # dwindle and master are both tiling modes
        dock_mode="tiling"
    fi
    
    echo "Detected layout: $layout, using dock mode: $dock_mode"
    
    # Kill existing dock first
    pkill -9 -f nwg-dock-hyprland 2>/dev/null
    sleep 0.8
    
    # Launch dock with proper mode
    ~/.config/hypr/scripts/dock-manager.sh "$dock_mode" &
    
    echo "Dock reloaded in $dock_mode mode"
fi
    sleep 0.3
