...
if [ -x ~/.config/hypr/scripts/dock-launcher.sh ]; then
    echo "Scheduling dock reload..."
    
    # Use systemd-run to spawn completely independent process
    systemd-run --user --scope --slice=app.slice \
        bash -c 'sleep 2; pkill -9 -f nwg-dock-hyprland; sleep 1; ~/.config/hypr/scripts/dock-launcher.sh' &
fi
