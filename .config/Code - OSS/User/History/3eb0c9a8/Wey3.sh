...
get_wallpaper_info() {
    ...
    # Try multiple methods to find current wallpaper
    CURRENT_WP=""
    ...
    # Method 2: Check for pywallpaper.jpg in main directory
    ...
    # Method 3: Search walls subdirectory
    if [ -z "$CURRENT_WP" ] || [ ! -f "$CURRENT_WP" ]; then
        CURRENT_WP=$(find "$WALLPAPER_DIR/walls" -maxdepth 1 -type f \( -name "*.jpg" -o -name "*.png" \) -printf '%T@ %p\n' 2>/dev/null | sort -rn | head -1 | cut -d' ' -f2-)
    fi
    ...
    if [ -n "$CURRENT_WP" ] && [ -f "$CURRENT_WP" ]; then
        ...
        WP_EXT="${WP_NAME##*.}"
        WP_SIZE=$(du -h "$CURRENT_WP" | cut -f1)
