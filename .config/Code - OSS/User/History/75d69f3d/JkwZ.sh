...
# Create always-visible config (windows won't overlap)
cat > "$DOCK_CONFIG" << 'EOF'
{
  "output": "",
  "layer": "overlay",
...
toggle_autohide() {
    ...
    if [ "$AUTO_HIDE" = "true" ]; then
        ...
        notify-send "󰘔 Dock" "Always Visible - Windows won't overlap" -i view-grid-symbolic
    else
...
## 4. Add Launcher Button Manually

**`~/.config/nwg-dock-hyprland/pinned`** - add this as first line:
```
rofi;~/.config/hypr/scripts/rofi-app-launcher.sh;/usr/share/icons/hicolor/scalable/apps/rofi.svg