...
#if [ -f ~/.config/hypr/scripts/sync-rofi-pywal.sh ]; then
    #~/.config/hypr/scripts/sync-rofi-pywal.sh
#fi
~/.config/hypr/scripts/sync-rofi-pywal.sh
~/.config/hypr/scripts/update-rofi-wallpaper.sh
...
if [ -x ~/.config/hypr/scripts/dock-launcher.sh ]; then
    ...
    # Simple background execution without blocking
    {
        sleep 1.5
        ...
        sleep 1
        if pgrep -f nwg-dock-hyprland > /dev/null; then
            echo "$(date): ✓ Dock reloaded" >> /tmp/dock-reload.log
        else
            echo "$(date): ✗ Dock failed" >> /tmp/dock-reload.log
        fi
    } >/dev/null 2>&1 &
fi
